...
enum RevenueCatConfig {
    ...
    static let apiKey = "appl_OgAKVcKNWSTOrcdpcKwBzNzUint"
}
...
final class SubscriptionService: NSObject, ObservableObject {
    ...
    /// There is one paid tier, so any active entitlement means Cribbage+.
    /// The fleet's canonical key is `pro`, but this project's entitlement is
    /// keyed `Cribbage+` and RevenueCat will not let a lookup_key be edited;
...
enum RevenueCatProbe {
    ...
    static let testStoreKey = "test_vooNZvAOmPMDyMkMFmmqQuGCWiQ"

    static var appUserID: String {
        ProcessInfo.processInfo.environment["RC_PROBE_USER"] ?? "funnel-probe-cribbage"
    }

    static var impressionID: String {
        ProcessInfo.processInfo.environment["RC_PROBE_SURFACE"] ?? "cribbage_onboarding_trial"
    }
}
#endif
