...
struct OnboardingView: View {
    ...
    @AppStorage("cribbage.skillLevel") private var skillLevel = ""
    ...
    private var pagesBody: some View {
        VStack(spacing: 0) {
            TabView(selection: $page) {
                ...
                infoPage(
                    ...
                    body: "Swipe through flashcards, read scoring shapes, make pegging calls, and choose two-card discards, with the why behind every answer.",
                    tiles: [.c(4), .d(5), .h(6), .s(7)]
                ).tag(1)
                infoPage(
                    icon: "figure.walk",
                    title: "Walk in confident",
                    body: "Know which cards make fifteens, spot scoring patterns fast, and stop dreading the discard. Practice at your own pace: no timers, no opponents.",
                    tiles: [.c(5), .d(10), .h(11)]
                ).tag(2)
        ...
        // Resolve the product before the price slot is on screen. The service
        // already asks at launch, but on a cold network the answer can still be
        // in flight when someone swipes through five pages in three seconds,
        // and the trial step has to show a real billed amount by then.
        .task(id: page) {
            await subscriptions.ensureOfferings()
        }
        .onChange(of: page) { _, newPage in
            guard newPage == lastPage else { return }
            subscriptions.trackPaywallImpression(id: "cribbage_onboarding_trial", oncePerSession: true)
        }
        .sheet(isPresented: $showPaywallFallback, onDismiss: paywallDismissed) {
            PaywallView(source: "cribbage_onboarding_fallback")
        }
    ...
    private let skillOptions: [SkillOption] = [
        ...
        SkillOption(id: "basics", title: "Know the basics", detail: "Know the deal, still slow on the count"),
            ...
            HStack(spacing: 12) {
                VStack(alignment: .leading, spacing: 3) {
                    ...
                    Text(option.detail)
                        .font(.subheadline)
                        .foregroundStyle(Theme.inkSecondary)
                        // Wrap rather than truncate. Cribbage's strings fit today,
