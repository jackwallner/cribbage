...
struct CribbageMinuteView: View {
    @StateObject private var store = CribbageMinuteStore.shared

    private var today: Date { Date() }
    private var challenge: CribbageMinuteChallenge { CribbageMinuteContent.challenge(for: today) }
    private var todayResult: CribbageMinuteResult? { store.result(for: today) }
    ...
    var body: some View {
        ...
        .navigationTitle("Cribbage Minute")
    ...
    private var hero: some View {
        VStack(spacing: 14) {
            ...
            VStack(spacing: 5) {
                Text("Today's Cribbage Minute")
                    .font(Theme.display(27))
                    .foregroundStyle(Theme.ink)
                Text("The same five original questions for every member: two hand reads, one discard decision, and two pegging calls.")
                    .font(.subheadline)
            ...
            if let todayResult {
                NavigationLink {
                    CribbageMinuteResultView(result: todayResult)
                } label: {
                    Text("View Today's \(todayResult.score)/\(todayResult.total)").primaryCTA()
                }
            } else {
                NavigationLink {
                    QuickSessionView(cribbageMinute: challenge)
                } label: {
    ...
    private func archiveRow(_ date: Date) -> some View {
        ...
        NavigationLink {
            if let result {
                CribbageMinuteResultView(result: result)
            } else {
                QuickSessionView(cribbageMinute: CribbageMinuteContent.challenge(for: date))
            }
        } label: {
            HStack(spacing: 12) {
...
struct CribbageMinuteResultView: View {
    ...
    private var scoreCard: some View {
        VStack(spacing: 8) {
            ...
            Text(result.score == result.total ? "Perfect minute!" : "Minute complete")
                ...
                .foregroundStyle(Theme.ink)
            Text("Cribbage Minute \(result.shortDate)")
                .font(.subheadline)
                .foregroundStyle(Theme.inkSecondary)
        }
        .frame(maxWidth: .infinity)
    ...
    private var breakdown: some View {
        VStack(alignment: .leading, spacing: 14) {
            Text("BY SKILL")
                ...
                .kerning(1.4)
                .foregroundStyle(Theme.inkSecondary)
            ForEach(CribbageMinuteCategory.allCases) { category in
                HStack(spacing: 12) {
    ...
    private func color(for category: CribbageMinuteCategory) -> Color {
    ...
    private func recordCompletionIfNeeded() {
        ...
        progress.recordSession(drillID: CribbageMinuteContent.drill.id)
