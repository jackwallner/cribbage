...
struct FlashcardDrillView: View {
    ...
    @AppStorage("cribbage.hasSwipedDeck") private var hasSwipedDeck = false
...
struct FlipCardFace: View {
    ...
    private var back: some View {
        CribbageCardFace(accent: accent, eyebrow: "THE CALL") {
            VStack(spacing: 14) {
                if let verdict {
...
/// The cribbage-card chrome both faces share: ivory surface, double frame like
/// the printed card, corner pips, and a faint suit watermark so the
/// open space feels designed instead of empty.
private struct CribbageCardFace<Content: View>: View {
    let accent: Color
    let eyebrow: String
