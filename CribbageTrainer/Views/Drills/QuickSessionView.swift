...
private enum QuickSessionPurpose {
    ...
    case dailyMinute(CribbageMinuteChallenge)
    case gameNightPrep

    var drill: Drill {
        switch self {
        case .quickSession: return SessionBuilder.sessionDrill
        case .dailyMinute: return CribbageMinuteContent.drill
...
struct QuickSessionView: View {
    ...
    init(cribbageMinute challenge: CribbageMinuteChallenge, onClose: (() -> Void)? = nil) {
    ...
    @State private var minuteResult: CribbageMinuteResult?
    ...
    private var completion: some View {
        if case .dailyMinute = purpose, let minuteResult {
            CribbageMinuteResultView(result: minuteResult, recordsCompletion: true, onDone: onClose)
        } else {
