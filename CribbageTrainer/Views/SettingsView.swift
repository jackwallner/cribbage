...
struct SettingsView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .sheet(isPresented: $showPaywall) { PaywallView(source: "cribbage_settings_sheet") }
            ...
            .alert("Notifications are off", isPresented: $settings.reminderPermissionDenied) {
                ...
                Text("Cribbage Trainer can't send your daily reminder until notifications are turned on for it in iOS Settings.")
            }
            .alert("Reset all progress?", isPresented: $showResetConfirm) {
                Button("Reset", role: .destructive) {
                    progress.resetAll()
                    PracticeRecordStore.shared.resetAll()
                    CribbageMinuteStore.shared.resetAll()
                }
    ...
    private var supportSection: some View {
        Section("Support") {
            ...
            Button {
                ...
                Label("Rate Cribbage Trainer", systemImage: "star.fill")
            }
            Button {
