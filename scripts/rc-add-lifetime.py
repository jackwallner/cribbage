...
Creates the com.jackwallner.cribbage.lifetime product on the App Store app record,
...
Usage: python3 scripts/rc-add-lifetime.py   (key read from ~/.cribbage_credentials)
"""
...
def secret_key() -> str:
    for line in (Path.home() / ".cribbage_credentials").read_text().splitlines():
        if line.startswith("RC_SECRET_KEY"):
            return line.split("=", 1)[1].strip().strip('"').strip("'")
    sys.exit("RC_SECRET_KEY not found in ~/.cribbage_credentials")
...
def main() -> None:
    ...
    project = projects[0] if len(projects) == 1 else next(
        p for p in projects if "cribbage" in p["name"].lower()
    )
    pid = project["id"]
